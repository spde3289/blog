...
export default function ReactRouterLearn() {
  return (
    <PostContainer>
      <PostHeader title='react-router에 대해서 알아보자' tagName='react-router' date='2023.02.09' />
      <Main>
        <SubTitle>react-router란?</SubTitle>
        <TextBox>
          react-router는 클라이언트 측 라우팅이 가능하게 도와주는 라이브러리다. <br />
          react-router를 사용하면 간단하게 url을 변경하면서 여러 페이지로 이루어진 어플리케이션을
          만들 수 있다.
        </TextBox>
    ...
    export default App;
        ...
        <TextBox>
          ...
          Route를 Routes로 감싸준다. Route에 path에는 url경로를 element에는 보여주고자 하는
          컴포넌트를 넣어준다. <br />
          그럼 path와 일치하는 Route를 렌더링 시켜주게 된다. path에 *만 적게 된다면 url에 일치하는
          경로가 없을시에 보여주게 된다.
        </TextBox>
        <CodeContainer>
          <pre>
    ...
    export default function Home() {
        ...
        <TextBox>
          기존에는 a태그를 사용하지만 react-router에서는 Link를 사용한다 사용방법은 아주 간단한데
          보이는거와 같이 <br />
          to에 원하는 경로를 적어주면 된다.
        </TextBox>
        <SubTitle>URL 파라미터</SubTitle>
        <TextBox>
          useParams를 이용하면 간단하게 파라미터를 받아올 수 있다. <br />
          path부분에 경로를 /contact/:Id 라고 한다면 /content 부분이 url /:Id부분이 파라미터라고 할
          수 있다.
        </TextBox>
        <CodeContainer>
          <pre>
