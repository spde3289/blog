...
export default function CustomHook() {
  ...
  return (
    <PostContainer>
      ...
      <Main>
        <TextBox>
          리엑트를 다룰 때 useState나 useEffect같은 Hook을 많이 사용해 봤을거다. 근데 이 Hook을 개발자가 
          원하는대로 만드는게 가능하다. 커스텀 훅은 반복되는 로직을 관리하는데 매우 용이하다. 
        </TextBox>
        <SubTitle>
          사용방법
        </SubTitle>
        <TextBox>
          커스텀 훅을 만드는 방법은 그렇게 어렵지 않다. 이전에 커스텀 훅을 만들때 지켜야 할 규직이 몇가지 있는데
          다음과 같다.
        </TextBox>
        <List>
        ...
        <TextBox>
          위에 있는 코드에서 중복되는 로직이 보일거다. 우리는 이제 이걸 커스텀 훅으로 만들어 재사용성을 
          높혀 보도록 하겠다.
        </TextBox>
        <CodeContainer>
          <pre>
