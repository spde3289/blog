...
export default function CreactContextErr() {
  return (
    <PostContainer>
      ...
      <Main>
        <TextBox>최근 타입스크립트 환경에서 creactContext를 사용하다 아래와 같은 에러가 발생해 createContext 사용방법과 해결방법을 정리해보려 한다.</TextBox>
        <CodeContainer>{code1}</CodeContainer>
        <SubTitle>creactContext 사용방법</SubTitle>
        <TextBox>기본적으로 creactContext를 생성해주면 된다.</TextBox>
        <CodeContainer>{code2}</CodeContainer>
        <TextBox>그 이후 구성 요소를 context.Provider 로 래핑 해준뒤 useContext로 호출해 사용할 수 있다.</TextBox>
        <CodeContainer>{code3}</CodeContainer>
        <TextBox>만약 기본값이 없다면 null으로 지정해주면된다.</TextBox>
        <CodeContainer>{code4}</CodeContainer>
        <TextBox>
          여기서 문제가 발생했는데 .Provider로 래핑해준 후 사용해주려 하니 위와 같은 에러가 발생했다. 이때 런타임 유형 검사를 통해 에러를 방지할 수 있다 혹은 {} as 빈 객체를 기본값으로 사용하고
