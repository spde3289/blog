  ...
  return (
    <PostContainer>
      ...
      <Main>
        ...
        <TextBox>
          위의 코드를 보면 받은 파라미터를 그대로 반환해 주는데 받은 파라미터 타입이 number가
          아니라면? <br />
          타입별로 함수를 만들어주게 되면 코드의 재사용성도 떨어지고 유지보수도 힘들어진다. 이떄
          사용하면 좋은게 제러릭이다.
        </TextBox>
        <CodeContainer>
          <code>
        ...
        <TextBox>
          만약 파라미터로 받은 값의 length를 확인 하고싶어 아래와 같이 작성하면 에러를 반환하게
          된다. <br />
        ...
        <TextBox>
          ts파일에서의 화살표 함수를 사용할때는 이렇게 사용하면 되는데 tsx파일에서 제너릭을 사용하게
          되면 (JSX element 'T' has no corresponding closing tag.) 같은 에러가 발생한다. {`(<>)`}를
          태그로 인식해 발생하는 문제인데 이때는 아래와 같이 사용하면 된다.
        </TextBox>
        <CodeContainer>
          <code>
            <pre>{code5}</pre>
        ...
        <br />
        <ReferenceLink
          href='https://joshua1988.github.io/ts/guide/generics.html#%EC%A0%9C%EB%84%A4%EB%A6%AD-%EC%A0%9C%EC%95%BD-%EC%A1%B0%EA%B1%B4'
          target='_blank'
        >
        ...
        <br />
        <ReferenceLink
          href='https://lakelouise.tistory.com/188#%F0%9F%93%9D-union-type'
          target='_blank'
        >
