# 🏷️ 블로그 버전 관리 및 배포 가이드

본 프로젝트는 `release-please` (v4)와 **Conventional Commits** 컨벤션을 기반으로 버전 부여, 체인지로그 작성, Vercel 배포를 100% 자동화하여 관리한다.

모든 버전 관리 규칙은 최상단의 `release-please-config.json` (규칙)과 `.release-please-manifest.json` (현재 상태 장부)에 의해 통제된다.
...
### ✨ MINOR (부 버전) : v1.1.0
기존 구조를 유지하면서 **새로운 기능이나 굵직한 변화가 추가**되었을 때 올린다.
* **적용 예시:**
...
### 🩹 PATCH (수 버전) : v1.1.1
기존 기능의 **버그를 수정하거나 아주 사소한 변경(콘텐츠 추가 포함)**이 일어났을 때 올린다.
* **적용 예시:**
  * **새로운 블로그 글 추가 (포스팅)**
...
* **`fix:`** (또는 PATCH 트리거로 설정된 접두사) 👉 **PATCH (수 버전) 상승**
  * **새로운 글 작성 (`fix: 리액트 훅 튜토리얼 포스팅 작성`)**
